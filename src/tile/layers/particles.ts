...
export class Particles extends TileLayer<ParticleProps_> {
  ...
  constructor(options: ParticleOptions_, source: WindSource) {
    ...
    this.pixelToGridRatio = 4;

  }
  public particleSpeed = 1;
  public dropRate = 0.01;
  public dropRateBump = 0.01

  private tileSize = 1024;
  private _numParticles = 65536;

  private updateProgram!: GlslProgram;
  private drawProgram!: GlslProgram;
  private screenProgram!: GlslProgram;

  private framebuffer: WebGLFramebuffer | null = null;
  private quadBuffer: WebGLBuffer | null = null;
  private nullTexture!: WebGLTexture;
  private nullTile?: { getTexture(): WebGLTexture; };
  ...
  private initializeParticles(gl: WebGLRenderingContext, count: number) {
   ...
   */
  protected override computeLoadableTiles() {
  ...
  private findAssociatedDataTiles(tile: Tile): DataTile | undefined {
    let t = tile;
    let found;
    let matrix = new window.DOMMatrix();
  ...
  private update(gl: WebGLRenderingContext, tile: ParticleTile, data: DataTile) {
  ...
  public override render(gl: WebGLRenderingContext, matrix: mat4) {
    if (this.windData) {

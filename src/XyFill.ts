...
export class XyFill extends WindGlLayer<XYFillProps> {
    constructor(options: XYFillOptions) {
        super(
            {
                "x-fill-color": {
                    type: "color",
    ...
    setXFillColor(expr: mb.StylePropertyExpression) {
        this.xExpr = expr;
        if (this.yExpr) this.makeColorGrid();
    }
    setYFillColor(expr: mb.StylePropertyExpression) {
        this.yExpr = expr;
        if (this.xExpr) this.makeColorGrid();
    }
    makeColorGrid() {
        this.colorGridTexture = this.buildColorGrid(this.xExpr!, this.yExpr!);
    }
